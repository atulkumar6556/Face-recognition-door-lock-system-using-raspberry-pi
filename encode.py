...
def encode():
    knownEncodings = []
    knownNames = []
    photos_list = glob.glob("id/*") # from id folder taking photos
    total_images = len(photos_list)
    s = time.time()
    for i in range(total_images):
        st = time.time()
        people_name = photos_list[i].split('/')[1].split('.')[0]
        people_id = i+1
        
        print("encoding People {0} named {1} with id {2}".format(i+1, people_name, people_id))# Load a sample picture and learn how to recognize it.
        people_image = face_recognition.load_image_file(photos_list[i])
        people_face_encoding = face_recognition.face_encodings(people_image , num_jitters=10)[0]
